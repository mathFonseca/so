...
typedef struct task_t
{
  ...
  int time_exec;   // Tempo total de execução.
  int time_proc;   // Tempo total dentro do processador.
  int n_ativ; // Número de ativações da tarefa até concluir.
  int created_time;    // Quantos ticks tinha o sistema quando a tarefa\
      executou pela primeira vez.
  int final_time;  // Quantos ticks tinha o sistema quando a tarefa terminou\
   de executar. (Ai dá pra ver o tempo total mais facilmente)
  int task_wait_id; // ID da Tarefa na qual (se existir), deve terminar primeiro\
  (usar no task_join)
  int exitCode; // Armazena o exitCode da tarefa.
